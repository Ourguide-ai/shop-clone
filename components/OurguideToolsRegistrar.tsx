...
/**
 * Returns the tools map built from current React context. Use this hook to
 * pass tools as a prop to ArgideProvider (React SDK path) and to register them
 * via window.ourguide (script-tag path) simultaneously.
 */
export function useOurguideTools(): Record<string, ToolHandler> {
...
/**
 * Registers tools via window.ourguide (script-tag / IIFE widget path).
 * Also keeps tools in sync with ArgideProvider via the tools prop (React SDK path)
 * — see ArgideProviderWithTools in layout.tsx.
 */
  ...
  useEffect(() => {
    const register = () => {
      if (typeof window.ourguide === "function") {
        console.log("[OurguideTools] Registering tools via window.ourguide.");
        window.ourguide("registerTools", tools);
        console.log("[OurguideTools] Tools registered successfully.");
      } else {
        console.log("[OurguideTools] window.ourguide not ready, retrying in 2000ms...");
